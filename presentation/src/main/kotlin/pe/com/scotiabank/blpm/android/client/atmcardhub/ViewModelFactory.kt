package pe.com.scotiabank.blpm.android.client.atmcardhub
...
class ViewModelFactory @Inject constructor(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>, extras: CreationExtras): T {
        if (modelClass.isAssignableFrom(AtmCardHostViewModel::class.java)) {
            return createViewModel(extras) as T
        }
        throw IllegalArgumentException("Unknown ViewModel Class: " + modelClass.name)
    }

    private fun createViewModel(extras: CreationExtras): AtmCardHostViewModel {

        val holder = HolderOfBooleanCreation(extras)

        return AtmCardHostViewModel(
            dispatcherProvider = dispatcherProvider,
            hubFactory = createHubFactory(),
            environmentHolder = environmentHolder,
            isDeepLink = holder.findBy(InstallmentConstants.DEEPLINK),
    ...
    private fun createVisitRegistry(): VisitRegistry {
        val maxNumberAllowedById: Map<Long, Int> = mapOf(
            AtmCardHostViewModel.HOST_ID_FOR_VISIT_REGISTRY to 1,
        )
